...
* In 29 colonies sampled in 2014 (Before ENSO), 2015 (during a 1st peak of heat) and 2016 (during 2nd peak of heat)
...
Add ORF Pocillopora and ITS2 dominant profile in 2014
...
# Create a data frame with initial (2014) ITS2 profiles and Pocillopora ORF types 
    ...
    # Colonies mainly hosting C1d (Pocillopora Type 3)
        ITS2_C1d<-data.frame("Colony"=c("256-1", "257-1", "260-1", "262-1", "330-1", "340-1A"),
                         "ITS2_14"="C1d", 
                         "ORF"="Type 3")
  ...
  data.Uva$ORFCommunity<-factor(data.Uva$ORFCommunity,
                                levels = c("Type 3_Cladocopium-only", "Type 1_Cladocopium-only",
                                           "Type 1_Mixed", "Type 1_Durusdinium-only"))
  summary(data.Uva$ORFCommunity)
...
Traject_Proportion_FacetORF <- ggplot (data.Uva) +
   ggthe_bw + 
    geom_line(aes (x=Year.2, D.Prp, group=Colony, colour=factor(ORF)))+ 
    facet_wrap(~ORF) +
    geom_jitter(aes(Year.2, D.Prp, colour=(Depht)),
              height = 0.05, width = 0.15) +
    #labs(x = "Año del muestreo", y = "Durusdinium Proportion") +
    theme(legend.position="none")
...
Traject_Proportion_FacetORF <- ggplot (data.Uva) +    ggthe_bw +
      geom_line(aes (x=Year.2, D.Prp, group=Colony, shape=(ORF)))+
...
SH_2014CoI<-lmerTest::lmer(logTot.SH ~ Year * ORF * DominantCommunity + (1|Colony), data=data.Uva)
    ...
    SH_I_groups<-SH_I_groups[order(
      SH_I_groups$Year,SH_I_groups$ORF, SH_I_groups$DominantCommunity),]
    SH_I_groups
    
    plot(emmeans(SH_2014CoI, ~Year|~ORF+DominantCommunity), comparisons = TRUE) +
      coord_flip(xlim = NULL, ylim = NULL, expand = TRUE) + facet_wrap(ORF~DominantCommunity) +ggthe_bw
    
    SH_Communities_I<-lmerTest::lmer(logTot.SH ~ Year * ORF * Community + (1|Colony), data=data.Uva)
    summary(SH_Communities_I)
    #step(SH_Communities_I)
    
    SH_CI.emmc<-emmeans(SH_Communities_I, ~Year*ORF*Community)
    SH_CI_groups<-cld(SH_CI.emmc)
    SH_CI_groups<-as.data.frame(SH_CI_groups[complete.cases(SH_CI_groups),])
    SH_CI_groups<-SH_CI_groups[order(SH_CI_groups$Year,SH_CI_groups$ORF, SH_CI_groups$Community),]
    SH_CI_groups
    #write.csv(SH_groups,"SH_Communities_multicomp.csv")
```   
...
SH_2014ComI <- ggplot(data.Uva, aes (Year.2, logTot.SH, colour=factor(DominantCommunity))) +  ggthe_bw + facet_wrap(~ORF) + ggtitle("a.")+
...
CH_Com <- ggplot(data.Uva, aes (Year.2, logC.SH, colour=factor(DominantCommunity))) + ggthe_bw+ ggtitle("b.")+
...
DH_ComI <- ggplot(data.Uva, aes (Year.2, logD.SH, colour=factor(DominantCommunity))) + 
      ggthe_bw+ ggtitle("c.")+
...
# ggsave(file="Outputs/Figure_3.svg", plot=Figure_3, width=7, height=11)
...
log_Log.data<-data.Uva[,c("Colony","Year.2", "C.SH", "D.SH", 
                          "D14Community", "Community","ITS2_14", "ORF")]
...
# ggsave(file="FigAndrewsFavoriteGraph_qPCRb.svg", plot=Community_Changes3, width=5.6, height=6)
...
```{r}
Community_Changes5 <- ggplot (log_Log.data, aes(logC, logD, gropup=Colony, colour=ORF)) +
