...
This file analyzes the changes in S/H cell ratio (bleaching and symboint shuffling) in *Pocillopora* spp.:
...
# 2. Uva Data
  ...
  # and 259 as well as 264 that were mislabeled in at least one of the sampling points   
    ...
    summary(data.Uva)
      ...
      ## Merge SH ratios and moratlity info
...
# qPCR BASED analysis (No ITS2 or ORF information included)

## 5a. Changes in the number of colonies dominated by each genus
...
### Considering initial (2014) community
...
### Figure 4: Cahnges in C/H and D/H  
...
### Add Pocillopora ORF lineage and Symbiodiniaceae ITS2 type hosted in 2014

```{r}
# Create a data frame with initial (2014) ITS2 profiles and Pocillopora ORF types 

    # Cladocopium dominated
    # Colonies mainly hosting C1d (Pocillopora Type 3)
        ITS2_C1d<-data.frame("Colony"=c("256-1", "257-1", "260-1", 
                                        "262-1", "330-1", "340-1A"),
                         "ITS2_14"="C1d", 
                         "ORF"="Type 3")
   # Colonies mainly hosting C1bc (Pocillopora Type 1)
        ITS2_C1bc<-data.frame("Colony"=c("265-1", "266-1", "268-1",  
                                         "270-1A", "271-1", "285-1",
                                         "287-1", "336-1A", "339-1A"),
                          "ITS2_14"="C1bc", 
                          "ORF"="Type 1") # 267 and 269 (~50% C and 50% D) under D1
    ...
    data.Uva$ORF<-factor(data.Uva$ORF, levels=c("Type 1", "Type 3"))
    
    # Summary ITS2/ORF combinations
  ColonyDistribution<-data.Uva[data.Uva$Year.2=="2014",]
                               
  SummaryColony<-ColonyDistribution%>%
        group_by(Year.2, Reef, ORF, ITS2_14) %>%
        dplyr::summarise(total.count=n())
  SummaryColony
  ...
  data.Uva$ORFCommunity<-factor(data.Uva$ORFCommunity,
                                levels = c("Type 3_Cladocopium-only", 
                                           "Type 1_Cladocopium-only",
                                           "Type 1_Mixed", 
...
Traject_Proportion_FacetORF <- ggplot (data.Uva) +    ggthe_bw +
      ...
      geom_jitter(aes(Year.2, D.Prp, colour=(Depht), shape=ORF),
              height = 0.00, width = 0.25, size=3) +  
      scale_colour_gradient(low = "red", high = "blue")
Traject_Proportion_FacetORF


# Traject_Proportion_FacetORF <- ggplot (data.Uva, aes(x=Depht, y=D.Prp, group=Colony)) +
#   ggthe_bw +
#   geom_line()+
#   geom_jitter(aes(shape=ORF, colour=Year),
#               height = 0.00, width = 1, size=3) 
# Traject_Proportion_FacetORF
```
...
Traject_ProportionITS2<- ggplot(data.Uva, aes (x=Year.2, y=D.Prp, group=(Colony)))+
        ...
        geom_jitter(aes(colour=Mor_ITS2, shape=Mortality),
                    height = 0.01, width = 0.25, alpha=0.9) +
...
CH_I<-lmer(logC.SH ~ Year* DominantITS2 + (1|Colony), data=data.Uva)
    summary(CH_I)
    #step(CH_I)
...
### Figure 3: qPCR by Pocillopora lineage

```{r}
Figure_3<-grid.arrange(SH_2014ComI, CH_Com2ITS2, DH_Com2ITS2,nrow=3)

Figure_3_InitailCommunity<-grid.arrange(CH_Com2ITS2, DH_Com2ITS2,nrow=2)
...
```{r}
Community_Changes2 <- ggplot (log_Log.data, aes(logC, logD, gropup=Colony, shape=ORF)) +
...
Community_Changes3 <- ggplot (log_Log.data, aes(logC, logD, gropup=Colony, colour=Colony)) +
  ...
  geom_point(aes(shape=ORF), alpha=0.5, size=4) +
  geom_path(size = 1,arrow =
              arrow(type = "open", angle = 30, length = unit(0.1, "inches")))+
...
# ggsave(file="FigAndrewsFavoriteGraph_Colony.svg", plot=Community_Changes3, width=5.6, height=6)

```

```{r}

Community_Changes4 <- ggplot (log_Log.data, aes(logC, logD, gropup=Colony, colour=ITS2_14, shape=ORF)) +
...
#ggsave(file="FigAndrewsFavoriteGraph_ITS2.png", plot=Community_Changes4, width=6, height=7.1)
``` 


```{r}
Community_Changes5 <- ggplot (log_Log.data, aes(logC, logD, gropup=Colony, colour=ITS2_14)) +
  ...
  scale_colour_manual(values=c("blue","purple", "red"),
                        guide = guide_legend(title.position = "top", nrow = 3),
                        name = "Dominant symbiont 2014)",
                        labels=c("C1d", "C1bc", "D1"))+
  ...
  annotate("text", x = c(seq(-5.1, -1.1, by=1)), y = -0.9, label = c(
    "10,0000D : 1C", "1,000D : 1C", "100D : 1C","10C : 1D", "C : D"), angle = 45)+
  annotate("text", x = -0.9, y = c(seq(-4.7, -1.7, by=1)), label = c(
    "1D : 10,000C", "1D : 1,000C", "1D : 100C",  "1D : 10C"), angle = 45)
  
Community_Changes5
#ggsave(file="FigAndrewsFavoriteGraph_ORF_ITS2.svg", plot=Community_Changes5, width=6, height=6)

```


```{r}
Community_Changes6 <- ggplot (log_Log.data, aes(logC, logD, gropup=Colony, colour=ITS2_14)) +
...
#ggsave(file="LogLog_TYMortality_labelpoints.svg", plot=Community_Changes6, width=6, height=7.1)
